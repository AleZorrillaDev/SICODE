...
def fetch_contracts_for_ruc(ruc):
    ...
    while page_number <= total_pages:
        ...
        try:
            ...
            # Manejar específicamente el error de NoneType si ocurriera
            error_msg = str(e)
            if "'NoneType' object has no attribute 'get'" in error_msg:
                error_msg = "Respuesta de SEACE no contiene datos válidos (Sin información en formato Exportar)"
            print(f"[!] Error procesando RUC {ruc}: {error_msg}")
            raise Exception(error_msg) # Relanzar para que process_rucs lo capture
    ...
    for contract in contracts:
        ...
        if contract_date and contract_date >= START_DATE:
            item = {
                ...
                "MONTO": contract.get("monto_del_contrato_original", ""),
                "OBJETO": contract.get("objeto", ""),
                "ESTADO": contract.get("estado", ""),
                "MIEMBROS CONSORCIO": contract.get("miembros_consorcio", "")
            }
...
def generate_excel_bytes(data):
    ...
    columns = [
        ...
        "MONTO", "OBJETO", "ESTADO", "MIEMBROS CONSORCIO"
    ]
